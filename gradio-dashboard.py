...
books["large_thumbnail"] = (
    books["thumbnail"].str.replace("http://", "https://", regex=False) + "&fife=w800"
)
...
if __name__ == "__main__":
    ...
    server_port = os.getenv("GRADIO_SERVER_PORT") or os.getenv("PORT")
    if server_port:
        launch_options["server_port"] = int(server_port)
    if server_name := os.getenv("GRADIO_SERVER_NAME"):
        launch_options["server_name"] = server_name
    dashboard.launch(**launch_options)
